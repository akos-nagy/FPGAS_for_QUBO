...
For the rest of the paper we assume that $\dom \( f \) = \Z_2^n$ and thus $U = H^{\otimes n}$. Furthermore, we note that $\QFT_d \ket{0}_d = H^{\otimes m} \ket{0}_d$ and thus we replace $\QFT_d$ with $H^{\otimes d}$ in the oracle, as the latter has depth $1$ and uses only single-qubit gates.
