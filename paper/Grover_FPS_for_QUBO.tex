...
\tikzstyle{process} = [ellipse, minimum width=3em, minimum height=2em, text centered, draw=blue, fill=gray!10]
...
In \cite{gilliam_grover_2021}, Gilliam et al. have proposed a marker oracle design for QUBO problems: given an instance of an integer QUBO problem, say, by $n$-by-$n$ upper-triangular integer matrix, $Q$, and a threshold $y \in \Z$, their oracle marks states $\ket{x}$ exactly when $x^T Q x \geqslant y$. This is precisely the ingredient needed to run Grover-type search algorithms to find configurations with values above the threshold. Furthermore, in the same paper Gilliam et al. proposes a \emph{Grover Adaptive Search} for QUBO problems. We explain their construction for the marker oracle in \Cref{sec:encoder} and their Grover Adaptive Search in \Cref{sec:adaptive}. This paper was inspired by these results of Gilliam et al. and our key results can be viewed as improvements on both their oracle design and adaptive method.
...
\subsection*{Organization of the paper:} In \Cref{sec:qdicts_and_qubos}, we introduce QUBO problems, outline of the construction of Gilliam et al. of the oracle for QUBOs \cite{gilliam_grover_2021}, and present our modified design. In \Cref{sec:grover_for_qubo}, we introduce the Fixed-point Grover Search and, using our oracle, apply it to QUBO problems. Finally, \Cref{sec:adaptive} considers the adaptive version of the Fixed-point Grover Search.
...
\begin{equation}
    Q = \begin{pmatrix}
        ...
        \end{pmatrix},
\end{equation}
with threshold $y = 5$ that is, in fact, the maximum. Out of the $2^5 = 32$ configurations only $3$ equal to the threshold, thus $\lambda = \tfrac{3}{32}$. The oracle was decomposed to $1$-qubit and CNOT gates, yielding a circuit depth of $47$ and a CNOT count of $66$. The experimental results are given in \Cref{table:oracle}.
...
In general, the implementation of $S_t \( \beta \)$ is case specific. In the case of an instance of a QUBO problem, let $T \eqdef \{ x \in \F_2^n | f(x) < 0 \}$. Now the results of \Cref{sec:encoder,sec:xor_basis} can be immediately used to get an oracle, $U_t$, on $n + 1$ qubits, such that for all $x \in \F_2^n$ and $y \in \F_2$, we have
\begin{equation}
...
\end{equation}
where $t$ is the elapsed time and $y$ is the higher known (found) value. Let $\mathrm{G} \( y, l \)$ be the FPGS for QUBO with threshold $y$, $\delta = 0.5659$, and $l$ queries. With the above arguments in mind, we propose an adaptive, quantum--classical hybrid method, shown on \Cref{figure:flowchart} below.
...
\begin{figure}[h!]
     ...
     \node[process] (start) {START: Choose $x \in \F_2^n$ randomly and set $\( t, l, y \) \eqdef \( 0, 1, f \( x \) \)$.};
     
     \node[process, below of=start, yshift= -4em] (FPGS) {Measure $\mathrm{G} \( y, \lceil l \rceil \) \ket{0}$ to get $x_\star \in \F_2^n$.};
     
     \node[process, below of=FPGS, yshift=-3em] (x_update) {$f \( x_\star \) > y$?};
     
     \node[process, below of=x_update, xshift=10em, yshift=-2em] (x_update_yes) {Update $\( x, y \) \mapsto \( x_\star, f \( x_\star \) + 1 \)$.};
     
     \node[process, below of=x_update_yes, xshift=-10em, yshift=-2em] (fork) {$T_f \( t, y \)$?};
     
     \node[process, below of=x_update_yes, xshift=10em, yshift=-2em] (l_update) {Update $t \mapsto t + \lceil l \rceil$ and $l \mapsto 1.523 \cdot l$.};
